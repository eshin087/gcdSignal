...
export default function CurationControls() {
  ...
  return <section aria-label="Reading preferences" className="mt-3 space-y-3 border-t border-zinc-200 pt-3 text-sm dark:border-zinc-800">
    ...
    <form className="flex flex-wrap items-center gap-2" onSubmit={(e) => { e.preventDefault(); const term = company.trim(); if (term) { setFollowedTerms([...followedTerms, term]); setCompany(""); } }}>
      <input aria-label="Follow a company or product" placeholder="Company or product, e.g. Anthropic" className="reader-input flex-1" maxLength={80} value={company} onChange={(e) => setCompany(e.target.value)} />
      <button className="action-button">Follow</button>
      {followedTerms.map((term) => <button type="button" className="action-button" key={term} onClick={() => setFollowedTerms(followedTerms.filter((t) => t !== term))} aria-label={"Unfollow " + term}>{term} ×</button>)}
    </form>
